...
model = AutoModelForSequenceClassification.from_pretrained('Mahendranath/nlp-ts', num_labels=1)


def generate(source_data_path):
    # Iterate over each company folder in SourceData
    for company_symbol in os.listdir(source_data_path):
        company_path = os.path.join(source_data_path, company_symbol)
        
        # Check if the item in SourceData is a directory (i.e., a company folder)
        if os.path.isdir(company_path):
            # Iterate over each year's Annual_reports folder
            for annual_report_folder in os.listdir(company_path):
